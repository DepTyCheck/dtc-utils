...
taskInvocation x                     = 
  failAt .| getFC x .| "Failed to extract invocation from lambda"
...
getTask : TypeTask l => l -> Name -> Elab MonoTask
...
unifyCon : (t : MonoTask) -> t.Con -> EitherT String Elab UnificationResult
unifyCon t con = do
  ...
  Right uniRes <- lift $ typeCheckUnifier uniTask
...
genArgAliases : List Arg -> List (Name, Name) -> Elab (List Arg, List (Name, Name))
...
genArgAliases ((MkArg count piInfo (Just name) type) :: xs) lnn = do
  ...
  let type = wrapInLets (mapSnd var <$> lnn) type
...
mkMultiInjDecls : MonoTask -> UniResults -> TypeInfo -> Elab $ List Decl
...
||| monomorphic equivalent
mkMultiCongDecl : UnificationResult -> Con aty ags -> Name -> Elab $ List Decl
...
||| a monomorphic equivalent
mkMultiCongDecls : MonoTask -> UniResults -> TypeInfo -> Elab $ List Decl
...
mkCastInjDecls : MonoTask -> UniResults -> TypeInfo -> Elab $ List Decl
...
mkDecEqDecls : MonoTask -> UniResults -> TypeInfo -> Elab $ List Decl
...
monoDecls : MonoTask -> UniResults -> TypeInfo -> Elab $ List Decl
...
public export
monomorphise : TypeTask l => l -> Name -> Elab (UniResults, TypeInfo, List Decl)
...
monomorphise' taskT outputName = do
  (uniResults, monoTy, decls) <- monomorphise taskT outputName
  declare decls
